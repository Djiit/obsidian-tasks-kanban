...
describe('groupTasks by priority', () => {
    it('splits into lanes by priority label, ordered alphabetically', () => {
        ...
        expect(labels(tasks, state({ field: 'priority' }))).toEqual(['High', 'Low']);
    });
...
describe('groupTasks path derivations', () => {
    it('groups by folder (trailing slash)', () => {
        const tasks = [createTask({ taskLocation: { path: 'work/projects/file.md', lineNumber: 1 } })];
        expect(labels(tasks, state({ field: 'folder' }))).toEqual(['work/projects/']);
    });
