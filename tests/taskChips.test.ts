...
describe('stripTags', () => {
    ...
    it('removes a nested tag as one token', () => {
        expect(
            stripTags('Deep task #work/project/sub', ['work/project/sub']),
        ).toBe('Deep task');
    });
...
describe('formatDate', () => {
    ...
    it('supports a Moment-like object', () => {
        const moment = { format: (fmt: string) => (fmt === 'YYYY-MM-DD' ? '2026-06-17' : '') };
...
describe('getDateChips', () => {
    ...
    it('does not flag a past-due done task as overdue', () => {
        ...
        const chips = getDateChips(task, now);
        expect(chips.find((c) => c.label === '2026-06-01')?.modifier).toBe('date-due');
    });
...
describe('getDependencyChips', () => {
    it('marks a task blocked by an open dependency', () => {
        const blocker = createTask({ id: 'design-schema', description: 'Design schema' });
    ...
    it('does not mark blocked when the dependency is done', () => {
        ...
        const task = createTask({ id: 'run-migrations', dependsOn: ['provision-db'] });
    ...
    it('counts multiple dependencies and blocks on any open one', () => {
        ...
        const task = createTask({ id: 'deploy', dependsOn: ['setup-ci', 'setup-cd'] });
    ...
    it('lists dependents in the id chip tooltip when others depend on it', () => {
        const task = createTask({ id: 'design-schema' });
        const dependent = createTask({ id: 'impl-api', description: 'Implement API', dependsOn: ['design-schema'] });
    ...
    it('reads the blockedBy fallback field', () => {
        ...
        const task = createTask({ id: 'consumer', dependsOn: undefined as unknown as string[], blockedBy: ['dep'] });
