...
export class TasksIntegration {
    ...
    async loadStatuses(): Promise<void> {
        const settings = this.readStatusSettings() ?? (await this.readStatusSettingsFromFile());
        if (!settings) {
            return;
        }
        ...
        const flattened = [
            ...
            .filter((s): s is TasksPluginStatus => Boolean(s) && typeof s.symbol === 'string')
            .map((s) => ({
                symbol: s.symbol,
                name: s.name,
                type: s.type,
    ...
    private async readStatusSettingsFromFile(): Promise<TasksStatusSettings | null> {
        try {
            ...
            const parsed = JSON.parse(raw) as { statusSettings?: TasksStatusSettings };
